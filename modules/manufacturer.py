        ...
        if request.method == 'POST':     
            ...
            if existe_email:
                ...
                if fabricante:     
                    ...
                    #resp = jsonify({"status":'success', "msj":"El negocio fue registrado con éxito","id_empresa":manufacturer.id_empresa})
                    #return sendResponse(resp)
                    print("files")
                    #print(request.files)
                    #register_files=registerBussinesFiles(request.files,manufacturer.id_empresa)
                    #if register_files:
                    #resp = jsonify({"status":'success', "msj":"El negocio fue registrado","id_empresa":manufacturer.id_empresa})  
...
@modules.route('/manufacturer/register-premium', methods=['POST'])
def registerManufacturerPremium():
    try:
        print("entro en /manufacturer/register-premium")
        manufacturer= Company() #Instancia  
        print("paso instancia")   
        _json=json.loads(request.values['data'])
        print(_json)      
        ...
        if request.method == 'POST':     
            ...
            if existe_email:
                ...
                if fabricante:     
                    ...
                    if documentosRM and documentosRIF and documentosCI and documentosRS:
                        send_mailCompanyCode(manufacturer.strcorreo,manufacturer.strnombre_empresa,)  
                        resp = jsonify({"status":'success', "msj":"El fabricante fue registrado con éxito"})
                        return sendResponse(resp)                                        
                    else:
...
def manufacturerValidate():
    try:
        if request.method == 'POST':   
            ...
            print(existe_manufacturer)
            if not existe_manufacturer==None:
